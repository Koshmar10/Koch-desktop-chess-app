import { ChessKnight, Clock, Flag, Flame, Play, Share2, Zap } from "lucide-react";
...
const MODE_OPTIONS: { mode: GameMode; label: string; time: string; icon: React.ReactNode }[] = [
  { mode: "Bullet", label: "bullet", time: "1 min", icon: <Flame size={ICON_SIZE} className="text-primary" /> },
  { mode: "Blitz", label: "blitz", time: "3 min", icon: <Zap size={ICON_SIZE} className="text-primary" /> },
  { mode: "Rapid", label: "rapid", time: "10 min", icon: <Clock size={ICON_SIZE} className="text-primary" /> },
  { mode: "Classical", label: "classical", time: "30 min", icon: <ChessKnight size={ICON_SIZE} className="text-primary" /> },
];
  ...
  return (
    <div className="flex flex-row gap-4 items-center mt-4">
      ...
      <Dropdown
        ...
        trigger={({ open, toggle }) => (
          <TooltipButton
            icon={selectedGameMode ? MODE_TRIGGER_ICON[selectedGameMode] : <Clock size={ICON_SIZE} />}
            ...
            {MODE_OPTIONS.map(({ mode, label, time, icon }, idx) => (
              <button
                ...
                className={`px-4 py-2 hover:bg-primary text-left flex justify-between items-center text-foreground ${
                  ...
                  selectedGameMode === mode ? "bg-primary font-bold" : "bg-primary/40"
                }`}
                onClick={() => {
